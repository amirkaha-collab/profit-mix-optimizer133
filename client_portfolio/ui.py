...
""", unsafe_allow_html=True)
    ...
    with st.expander(
            ...
            if uploaded:
                ...
                try:
                    ...
                    if all_recs:
                        ...
                        added = import_from_session(st, df_long, "אחר")
                        holdings_raw = _get_pf_holdings()  # refresh
                        imported_msg = f" | יובאו לניתוח: {added}" if added is not None else ""
                        st.success(f"✅ טעון: {len(all_recs)} מוצרים, ₪{total:,.0f}{imported_msg}")
                        st.rerun()
                    else:
        ...
        if raw_import:
            ...
            if new_ct > 0:
                st.info("הקובץ נטען אך טרם יובא לניתוח. העלאה חדשה תייבא אותו אוטומטית.")
            else:
    ...
    if not holdings_raw:
        st.info("💡 העלה דוח מסלקה למעלה או הוסף מוצרים ידנית.")
        return
