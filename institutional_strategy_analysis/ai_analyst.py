...
──────────────────────────────────────────────
Builds rich analytical prompts and calls OpenAI for deep institutional
...
def _call_claude(prompt: str, system: str = "", max_tokens: int = 3200,
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return "", "מפתח OpenAI לא תקין — בדוק שהמפתח נכון."
            ...
            try:
                ...
                err = {}
            msg = ((err.get("error") or {}).get("message") or "").strip()
            return "", f"שגיאת OpenAI: HTTP {resp.status_code}" + (f" — {msg}" if msg else "")
...
_SYSTEM = (
    "אתה מנהל השקעות בכיר ומנתח אסטרטגיות מוסדיות ברמה בינלאומית — "
    "רמה של CIO בקרן פנסיה גדולה או ראש צוות manager selection בבית השקעות "
    "מהשורה הראשונה (BlackRock, Goldman Sachs, Vanguard). "
    "אתה מתמחה בגופים מוסדיים ישראלים — קרנות פנסיה, קרנות השתלמות, קופות גמל. "
    "ניתוחך מיועד למנהלים פיננסיים, יועצי השקעות ובוחרי מנהלים. "
    "הסגנון: עברית מקצועית גבוהה, תמציתית, ממוקדת בהחלטות. "
    "ציין נתונים מספריים מדויקים מתוך הקלט. "
    "השתמש בטרמינולוגיה מוסדית: relative positioning, style drift, "
    "consistency, momentum, hedging ratio, manager selection. "
...
---
כתוב ניתוח שוק מקצועי — ניתוח יחסי בין כלל הגופים —
עם הכותרות הבאות בדיוק (##):

## מיצוב יחסי לפי רכיב
לכל רכיב (חו"ל, מניות, מט"ח, לא-סחיר) — בנפרד:
• מי גבוה ומי נמוך, בכמה pp, מה המשמעות האסטרטגית
• האם יש outlier בולט? מה מבדיל אותו?
• מה ממוצע הקבוצה וסטיית התקן הבין-גופית?

## דינמיות ואפיון סגנון ניהול
• מי תזזיתי (tactical) ומי עקבי (consistent) — ביחס לממוצע הקבוצה
• האם שינויים מגיעים יחד (קולקטיביים) או אינדיבידואלים?
• מה המשמעות לחוסך — תזזיתיות כאינדיקציה לסגנון CIO

## אסטרטגיית גידור מט"ח
• לכל גוף: יחס גידור, חשיפה נטו, מה מספרת הגישה על תפיסת הסיכון
• מי מגדר אגרסיבי, מי נותן לסיכון המטבעי לרוץ

## תנועות פוזיציה אחרונות (3–12 חודשים)
• מה זז משמעותית? מי שינה כיוון?
• האם יש מגמה קולקטיבית? מי מוביל ומי מפגר?

## ניתוח סיכון קבוצתי
• תנודתיות, חשיפה ללא-סחיר, חריגות קיצון
• מי נמצא בחשיפת סיכון גבוהה ביחס לקבוצה?

## יתרונות וחסרונות לפי גוף
לכל גוף — בנפרד: יתרון ראשי וחסרון ראשי, מבוססי נתונים.

## תובנה אסטרטגית וסיכום
• מה מספרים הנתונים על כיוון השוק?
• לאיזה פרופיל חוסך מתאים כל גוף?"""
...
{'='*60}
הנחיות לניתוח — ברמת CIO / Manager Selection:
{'='*60}

כתוב ניתוח מיקוד מקצועי ומעמיק עם הכותרות הבאות בדיוק (##):

## מיצוב יחסי (Relative Positioning)
לכל אחד מהרכיבים — חו"ל, מניות, מט"ח, לא-סחיר — בנפרד:
• כמה {manager} גבוה/נמוך מהממוצע ומהמדיאן? ציין pp מדויק.
• מה המשמעות האסטרטגית — מה אומרת פוזיציה זו על ה-CIO?
• האם זהו outlier בולט, או מיצוב עקבי לאורך זמן?
• מה קרה בפוזיציה ב-3–12 חודשים האחרונים?

## ניתוח היסטורי עצמי (Historical Self-Analysis)
• מה היה מיצוב הגוף לפני 3–5 שנים לעומת היום — ציין מספרים.
• האם ישנו style drift מהותי? אם כן — לאיזה כיוון ולמה?
• זיהוי turning points משמעותיים — מתי ואיפה הגוף שינה פילוסופיה?
• עקביות ניהולית: האם הגוף נאמן למדיניות שלו לאורך זמן?

## סגנון ניהול ועקביות (Management Style & Consistency)
• האם הגוף tactical (מגיב לשוק) או strategic (פילוסופיה יציבה)?
• השוואת תזזיתיות לממוצע הקבוצה — מה המשמעות?
• pattern זיהוי: האם הגוף מגיב לאירועי שוק? counter-cyclical?
• ניתוח consistency — כמה הגוף שומר על target allocation?

## אסטרטגיית גידור ומטבע (Hedging & Currency Strategy)
• ניתוח יחס הגידור ביחס לעמיתים — מי אגרסיבי/שמרני יותר?
• חשיפת מטבע נטו — מה ההשלכה בתרחיש היחלשות/התחזקות שקל?
• האם הגידור עקבי או אדפטיבי לשינויי שוק?

## מומנטום ותנועות אחרונות (Recent Momentum)
• מה השתנה ב-3–6 חודשים האחרונים — ציין מספרים.
• האם הגוף unique בתנועות אלה ביחס לעמיתים?
• לאן פני הגוף? מה המגמה המתגבשת?
