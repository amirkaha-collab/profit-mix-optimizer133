...
def infer_product_type(text: str, default: str = "אחר") -> str:
    """Infer product type from free-text product/fund name."""
    s = str(text or "").strip().lower()
    if not s:
        return default

    checks = [
        (("קופת גמל להשקעה", "גמל להשקעה", "investment provident", "provident investment"), "גמל להשקעה"),
        (("קרן השתלמות", "השתלמות", "training fund"), "קרנות השתלמות"),
        (("פוליסת חיסכון", "פוליסת", "פוליסה", "saving policy"), "פוליסות חיסכון"),
        (("קרן פנסיה", "פנסיה", "pension"), "קרנות פנסיה"),
        (("ביטוח מנהלים", "מנהלים", "executive insurance"), "ביטוח מנהלים"),
        (("קופת גמל", "קופת", "גמל", "תגמולים", "פיצויים", "provident"), "קופות גמל"),
    ]
...
def import_from_session(st, df_long: pd.DataFrame, product_type: str) -> int:
    ...
    for r in raw:
        ...
        inferred_product_type = infer_product_type(
            f"{r.get('fund', '')} {r.get('track', '')}",
            default=product_type or "אחר",
        )
